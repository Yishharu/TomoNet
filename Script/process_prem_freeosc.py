...
def read_seismogram(Run_dir):
    ...
    BallRadius = 6371
    nlat, nlon = 19, 37
    StartTime, EndTime = 0.0, 6000.0   # note in Second
    resample_rate = 1.0    # note in Second
    lowpass_freq = 1/30.    # note in Hz
    ...
    for rank in rank_station_dict.keys():
        ...
        for [StationName, inrank_index] in rank_station_dict[rank]:
            ...
            ilat, ilon = int((lat+90)/10), int((lon+180)/10)
            colat = 90 - lat
...
def save_seismogram_hdf5(StartNum,EndNum):
    ...
    '''
    path = '../FinishedRuns'
    model_name = "LatinSphericalHarmonicsPREM"
    ...
    disp_dims = (num_models, 19, 37, 3, 6000)
    station_coords_cartesian_dims = (19, 37, 3) # nlat, nlon, ncomponent
    station_coords_spherical_dims = (19, 37, 3)
    time_dims = (6000) # PointPerTrace
    # source_dims = (num_models, 9)
    harmonics_dims = (num_models, 5427)
    ...
    with h5py.File(hdf5_file_path, "w") as data_hdf5:
        ...
        lat = np.radians(90 - np.linspace(-90, 90, 19)) # note this is colatitude
        lon = np.radians(np.linspace(-180, 180, 37))
        LON, LAT = np.meshgrid(lon, lat)
        radius = np.ones((19,37))*6371000
        ...
        # make time data
        time_data[:] = np.linspace(0,5999,6000)
...
def read_fourier_coef(Run_dir):
    nelement_mantlecrust = 12096
    nelement_outercore = 2052
    nelement_innercore = 2322

    fourier_dim = 16

    whole_disp_coef = np.ndarray((nelement_mantlecrust+nelement_outercore+nelement_innercore, 16, 3, 20))

    # mantle&crust
    na_grid, data_time, list_element_na, list_element_coords, \
    dict_list_element, dict_data_wave = read_element_output(f"{Run_dir}/output/elements/full_coeff_mantlecrust")
    ...
    indices = [mapping[tuple(point)] for point in coords_mantlecrust_ref]
    coords = coords[indices]
    assert (coords == coords_mantlecrust_ref).all()
    whole_disp_coef[0:nelement_mantlecrust,:,:,:] = disp_coef[indices,:,:,:]
    # print('mantle&crust OK')

    # outer core
    na_grid, data_time, list_element_na, list_element_coords, \
    dict_list_element, dict_data_wave = read_element_output(f"{Run_dir}/output/elements/full_coeff_outercore")

    disp_coef = np.squeeze(dict_data_wave[fourier_dim],axis=(2,))
    nelem, ngll = list_element_coords.shape[0], list_element_coords.shape[1]
    coords = list_element_coords.reshape((nelem * ngll), 2)
    mapping = {tuple(point): index for index, point in enumerate(coords)}
    indices = [mapping[tuple(point)] for point in coords_outercore_ref]
    coords = coords[indices]
    assert (coords == coords_outercore_ref).all()
    whole_disp_coef[nelement_mantlecrust:nelement_mantlecrust+nelement_outercore,:,:,:] = disp_coef[indices,:,:,:]
    # print('outer core OK')

    # inner core
    na_grid, data_time, list_element_na, list_element_coords, \
    dict_list_element, dict_data_wave = read_element_output(f"{Run_dir}/output/elements/full_coeff_innercore")

    disp_coef = np.squeeze(dict_data_wave[fourier_dim],axis=(2,))
    nelem, ngll = list_element_coords.shape[0], list_element_coords.shape[1]
    coords = list_element_coords.reshape((nelem * ngll), 2)
    mapping = {tuple(point): index for index, point in enumerate(coords)}
    indices = [mapping[tuple(point)] for point in coords_innercore_ref]
    coords = coords[indices]
    assert (coords == coords_innercore_ref).all()
    whole_disp_coef[nelement_mantlecrust+nelement_outercore:nelement_mantlecrust+nelement_outercore+nelement_innercore,:,:,:] = disp_coef[indices,:,:,:]
    # print('inner core OK')
...
def save_wffourier_hdf5(StartNum,EndNum):
    ...
    '''
    global coords_mantlecrust_ref, coords_outercore_ref, coords_innercore_ref
    path = '../FinishedRuns'
    model_name = "LatinSphericalHarmonicsPREM"
    hdf5_file_path = f"../DataSet/wf_fourier_data_{StartNum:0>4d}_{EndNum:0>4d}.h5"
    num_models = EndNum-StartNum  # Total number of models

    element_coords_sz_dims = (16470, 2)
    disp_coef_dims = (num_models, 16470, 16, 3, 20)
    harmonics_dims = (num_models, 5427)
    timestep_dims = (num_models, 20)
    ...
    with h5py.File(hdf5_file_path, "w") as data_hdf5:
        ...
        na_grid, data_time, list_element_na, list_element_coords, \
        dict_list_element, dict_data_wave = read_element_output(f"{path}/LatinSphericalHarmonicsPREM0000/output/elements/full_coeff_mantlecrust")
        nelem, ngll = list_element_coords.shape[0], list_element_coords.shape[1]
        ...
        na_grid, data_time, list_element_na, list_element_coords, \
        dict_list_element, dict_data_wave = read_element_output(f"{path}/LatinSphericalHarmonicsPREM0000/output/elements/full_coeff_outercore")
        nelem, ngll = list_element_coords.shape[0], list_element_coords.shape[1]
        ...
        na_grid, data_time, list_element_na, list_element_coords, \
        dict_list_element, dict_data_wave = read_element_output(f"{path}/LatinSphericalHarmonicsPREM0000/output/elements/full_coeff_innercore")
        nelem, ngll = list_element_coords.shape[0], list_element_coords.shape[1]
        ...
        # merge into whole
        element_coords_sz_data[:,:] = np.concatenate((coords_mantlecrust_ref, coords_outercore_ref, coords_innercore_ref), axis=0)[:,:]
...
def read_slice_data(Run_dir):
    ...
    nelement_innercore = 2322
    nslice = 16
    whole_wave_disp = np.ndarray((nelement_mantlecrust+nelement_outercore+nelement_innercore, nslice, 3, 20))

    # read
    na_grid, data_time, list_element_na, list_element_coords, \
    dict_list_element, dict_data_wave = read_element_output(f"{Run_dir}/output/elements/azimuthal_slices_mantlecrust")
    ...
    coords = coords[indices]
    assert (coords == coords_mantlecrust_ref).all()
    whole_wave_disp[0:nelement_mantlecrust,:,:,:] = wave_disp[indices,:,:,:]
    # print('mantle&crust OK')

    # outer core
    na_grid, data_time, list_element_na, list_element_coords, \
    dict_list_element, dict_data_wave = read_element_output(f"{Run_dir}/output/elements/azimuthal_slices_outercore")
    ...
    coords = coords[indices]
    assert (coords == coords_outercore_ref).all()
    whole_wave_disp[nelement_mantlecrust:nelement_mantlecrust+nelement_outercore,:,:,:] = wave_disp[indices,:,:,:]
    # print('outer core OK')

    # inner core
    na_grid, data_time, list_element_na, list_element_coords, \
    dict_list_element, dict_data_wave = read_element_output(f"{Run_dir}/output/elements/azimuthal_slices_innercore")
    ...
    coords = coords[indices]
    assert (coords == coords_innercore_ref).all()
    whole_wave_disp[nelement_mantlecrust+nelement_outercore:nelement_mantlecrust+nelement_outercore+nelement_innercore,:,:,:] = wave_disp[indices,:,:,:]
    # print('inner core OK')
...
def save_wfslice_hdf5(StartNum,EndNum):
    ...
    element_coords_sz_dims = (16470, 2)
    disp_dims = (num_models, 16470, 16, 3, 20) # model_id, element_id, slice_id, component_id, time_id
    harmonics_dims = (num_models, 5427)
    slice_dims = (16)
    ...
    with h5py.File(hdf5_file_path, "w") as data_hdf5:
        ...
        # sort mantle&crust element (12096, 2)
        na_grid, data_time, list_element_na, list_element_coords, \
        dict_list_element, dict_data_wave = read_element_output(f"{path}/LatinSphericalHarmonicsPREM0000/output/elements/azimuthal_slices_mantlecrust")
        nelem, ngll = list_element_coords.shape[0], list_element_coords.shape[1]
        coords_mantlecrust_ref = list_element_coords.reshape((nelem * ngll), 2)
        dist = np.sum(coords_mantlecrust_ref**2, axis=1)
        idx = np.argsort(dist)
        ...
        # sort outer core element (2052, 2)
        na_grid, data_time, list_element_na, list_element_coords, \
        dict_list_element, dict_data_wave = read_element_output(f"{path}/LatinSphericalHarmonicsPREM0000/output/elements/azimuthal_slices_outercore")
        nelem, ngll = list_element_coords.shape[0], list_element_coords.shape[1]
        coords_outercore_ref = list_element_coords.reshape((nelem * ngll), 2)
        dist = np.sum(coords_outercore_ref**2, axis=1)
        idx = np.argsort(dist)
        ...
        # sort inner core element (2322, 2)
        na_grid, data_time, list_element_na, list_element_coords, \
        dict_list_element, dict_data_wave = read_element_output(f"{path}/LatinSphericalHarmonicsPREM0000/output/elements/azimuthal_slices_innercore")
        nelem, ngll = list_element_coords.shape[0], list_element_coords.shape[1]
        coords_innercore_ref = list_element_coords.reshape((nelem * ngll), 2)
        dist = np.sum(coords_innercore_ref**2, axis=1)
        idx = np.argsort(dist)
